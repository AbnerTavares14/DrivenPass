...
async function getCredential(id: number, userId: number) {
    ...
    const data: Credentials = { id: credential.id, url: credential.url, title: credential.title, username: credential.username, password: decryptedPassword, userId: credential.userId };
    return data;
}
